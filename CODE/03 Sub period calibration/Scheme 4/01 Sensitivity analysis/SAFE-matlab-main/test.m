...
%保存图片到以num命名的文件夹
pathname = fullfile('..\..\..\..\Scheme3\SA\', num2str(num));
if ~exist(pathname, 'dir')
    mkdir(pathname);
end
pathfig1 = fullfile(pathname, 'main_effects.png');
pathfig2 = fullfile(pathname, 'total_effects.png');

figure % plot main and total separately
subplot(121); boxplot1(Si,X_Labels,'main effects')
saveas(gcf, pathfig1); %保存图片到以num命名的文件夹

subplot(122); boxplot1(STi,X_Labels,'total effects')
saveas(gcf, pathfig2); %保存图片到以num命名的文件夹

figure % plot both in one plot:
boxplot2([Si; STi],X_Labels)

%保存图片到以num命名的文件夹
pathfig3 = fullfile(pathname, 'main_and_total_effects.png');
saveas(gcf, pathfig3); %保存图片到以num命名的文件夹
...
%保存图片到以num命名的文件夹
pathfig4 = fullfile(pathname, 'main_and_total_effects_with_confidence_bounds.png');
saveas(gcf, pathfig4); %保存图片到以num命名的文件夹
...
%保存图片到以num命名的文件夹
pathfig5 = fullfile(pathname, 'main_effect_convergence_with_confidence_bounds.png');
saveas(gcf, pathfig5); %保存图片到以num命名的文件夹

%保存.mat文件到以num命名的文件夹
datafile = fullfile(pathname, 'data.mat');
save(datafile, 'Si', 'STi', 'Si_sd', 'STi_sd', 'Si_lb', 'STi_lb', 'Si_ub', 'STi_ub', 'Si', 'STi', 'Si_sdc', 'STi_sdc', 'Si_lbc', 'STi_lbc', 'Si_ubc', 'STi_ubc');
