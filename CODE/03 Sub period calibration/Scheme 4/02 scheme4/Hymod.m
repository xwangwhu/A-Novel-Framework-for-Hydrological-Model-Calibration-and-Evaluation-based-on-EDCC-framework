...
%---hymod_parameters
    ...
    Huz=zeros(hymod.date.nDays,1);
    for i=1:nclust
        Huz(hymod.date.ID{i+1})=parameters(i);
    end
...
%     hymod.parameters.Huz=parameters(1);      % Maximum height of soil moisture accounting tank - Range [0, 1000]
    hymod.parameters.B=parameters(nclust+1);        % Scaled distribution function shape parameter    - Range [0,    2]
    hymod.parameters.alpha=parameters(nclust+2);    % Quick/slow split parameter                      - Range [0, 0.99]
    hymod.parameters.Nq=3;                   % Number of quickflow routing tanks               - Range [1, Inf] (typically set to <3)
    hymod.parameters.Kq=parameters(nclust+3);       % Quickflow routing tanks' rate parameter         - Range [0, 0.99]
    hymod.parameters.Ks=parameters(nclust+4);
    % Snow parameters (degree-day model)
    hymod.parameters.DDF=parameters(nclust+5);      % Degree day factor                               - Range [ 0, 2]
    hymod.parameters.Tth=parameters(nclust+6);      % Temperature threshold                           - Range [-5, 5]
    hymod.parameters.Tb=parameters(nclust+7);       % Base temperature to calculate melt              - Range [-5, 5]
...
%---Run Model for Simulation Period
    ...
    for modelDay=1:nDays 
        hymod.parameters.Huz=Huz(modelDay);       % Quickflow routing tanks' rate parameter         - Range [0, 0.99]
        ...
        % Make constraint judgment, once Huc<XHuz, break.
        if hymod.parameters.Huz<hymod.states.XHuz(modelDay)
            break
        end
