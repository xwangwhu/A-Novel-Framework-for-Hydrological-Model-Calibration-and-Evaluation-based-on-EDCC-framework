...
%---hymod_parameters
    ...
    alpha=zeros(hymod.date.nDays,1);
    for i=1:nclust
        alpha(hymod.date.ID{i+1})=parameters(i+3);
    end
...
%---Run Model for Simulation Period
    ...
    for modelDay=1:nDays 
        hymod.parameters.alpha=alpha(modelDay);       % Quickflow routing tanks' rate parameter         - Range [0, 0.99]
