...
%---hymod_parameters
    ...
    for i=1:nclust
        para.Huz(hymod.date.ID{i+1})=parameters(i,1);      % Maximum height of soil moisture accounting tank - Range [0, 1000]
        para.B(hymod.date.ID{i+1})=parameters(i,2);        % Scaled distribution function shape parameter    - Range [0,    2]
        para.alpha(hymod.date.ID{i+1})=parameters(i,3);    % Quick/slow split parameter                      - Range [0, 0.99]
        para.Kq(hymod.date.ID{i+1})=parameters(i,4);       % Quickflow routing tanks' rate parameter         - Range [0, 0.99]
        para.Ks(hymod.date.ID{i+1})=parameters(i,5);       % Slowflow routing tank's rate parameter          - Range [0, 0.99]
        % Snow parameters (degree-day model)
        para.DDF(hymod.date.ID{i+1})=parameters(i,6);
        para.Tth(hymod.date.ID{i+1})=parameters(i,7);
        para.Tb(hymod.date.ID{i+1})=parameters(i,8);
    end

    
%---Initialization  
        nDays=length(hymod.date.ID{1, 1});
