function [bestx,bestf,Parameters,Functions] = sceua(x0,bl,bu,maxn,kstop,pcento,peps,ngs,iseed,iniflg)

% This is the subroutine implementing the SCE algorithm, 
% written by Q.Duan, 9/2004
%
% Definition:
%  x0 = the initial parameter array at the start(初始参数向量);
%  bestx = the optimized parameter array at the end（最终优化得出的参数向量）;
%  f0 = the objective function value corresponding to the initial parameters（与初始参数相对应的目标函数值）
%  bestf = the objective function value corresponding to the optimized parameters（与最终优选参数相对应的目标函数值）
%  bl = the lower bound of the parameters（初始参数向量的下边界）
%  bu = the upper bound of the parameters（初始参数向量的上边界）
...
while icall<maxn & gnrng>peps & criter_change>pcento
    ...
    for igs = 1: ngs
        ...
        for loop=1:nspl
            ...
            [snew,fnew,icall,Parameters,Functions]=cceua(s,sf,bl,bu,icall,maxn,Parameters,Functions);
